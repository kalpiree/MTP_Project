...
if (strcmp(User_Choice,'1')==1)
    ...
    for sacle_idx=2:6
         ...
         GT_ComputeGaborIR_Frontal_jpeg_exp(sacle_idx); %gabor features computed for a particular scale
         GT_Gabor_HMFeatures_between_lt_and_rt_Frontal_jpeg_exp(sacle_idx); %d append gabor features calculated from each facial half
...
elseif (strcmp(User_Choice,'3')==1)
    ...
    %---For each scale, gabor features are calculated for each half of the face. Features from each half of face is appended and saved.---%
    for sacle_idx=2:6
        GT_ComputeGaborIR_Profile_jpeg_exp(sacle_idx);
        GT_Gabor_HMFeatures_between_lt_and_rt_haralic_Profile_jpeg_exp(sacle_idx);
