function [ROI_o,ROI_O] = side_face(lt,rt)
% =========================== Profile_keypoints.m ====================================== %
% Description  : 
...
%for folder_idx=no_dir:no_dir
    ...
    %dir_1="C:\Users\ntnbs\Downloads\ThermalDatabase\ThermalDatabase\dataset new\Malignant";
...
%% 
    ...
    %%%% get the ear hotspot row and column specified by ear_row, ear_col
     ...
     [mask,row,col,ROI_L,ROI_o] = modified_mask(tip_row,tip_col,canthus_r,canthus_c,nw_row,nw_col,ear_row,ear_col,chin_row, chin_col,mask,originalimg);
...
%%  
    ...
    %%%%%%%%%% mask generation using these points
    [mask,row,col,ROI_R,ROI_O] = modified_mask(tip_row,tip_col,canthus_r,canthus_c,nw_row,nw_col,ear_row,ear_col,chin_row, chin_col,mask,originalimg);
...
%if ~exist('output','dir')
 ...
 %figure,imshow(ROI_R);
 %figure,imshow(ROI_O);
 %% AN ATTEMPT TO EXTRACT ROI
 %os_pentagon = [canthus_c canthus_r ear_col ear_row ear_col chin_row chin_col chin_row nw_col nw_row];
 %c = [canthus_c ear_col ear_col chin_col nw_col];
 %r = [canthus_r ear_row chin_row chin_row nw_row];
 %ROI_R = flip(ROI_R,2);
 %ROI_R = flip(ROI_R,2);
 %BW = roipoly(ROI_R,c,r);
 %BW = flip(BW,2);
 %[p,q]=size(BW)
 %BW_1 = uint8(BW) .* uint8(ROI_R);
 %subplot(1,3,3);
 %figure,imshow(BW);
 %figure,imshow(BW_1);
 %%
 %image3= ROI_R(:,:,1);  % incorrect
 %subplot(1,3,3);
 %imshow(image3);
%
%imsave('A_profile_Mask');imsave('profile_mask_lt');imsave('profile_mask_rt');
%imsave('A_face_mask');
%imsave('img_face_lt');
%imsave('img_face_rt');
 
 %subplot(1,8,3);
 %imshow(A_profile_Mask);
