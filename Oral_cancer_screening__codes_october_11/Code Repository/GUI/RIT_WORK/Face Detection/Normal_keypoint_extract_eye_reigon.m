...
%     hold on;
 ...
 max_rt=max(rt_eye(:));
 %[max_rt_row,max_rt_col]=(find(max_rt==face_img));
 [max_rt_row,max_rt_col]=(find(max_rt==rt_eye));
 max_lt=max(lt_eye(:));
% [max_lt_row,max_lt_col]=(find(max_lt==face_img));
 [max_lt_row,max_lt_col]=(find(max_lt==lt_eye));
 figure();
 imshow(face_img);
 hold on;
%  plot(max_lt_col,max_lt_row,'c*');
%  plot(max_rt_col,max_rt_row,'c*');
row_rt_plot=min(max_rt_row+r_eye_reigon_min);
row_lt_plot=min(max_lt_row+r_eye_reigon_min);
col_rt_plot=min(max_rt_col+cmin);
col_lt_plot=min(max_lt_col+cmid);
plot(col_rt_plot,row_rt_plot,'c*');
plot(col_lt_plot,row_lt_plot,'c*');
mean_row=ceil((row_rt_plot+row_lt_plot)/2);
p1 = [row_rt_plot;col_rt_plot];
p2 = [row_lt_plot;col_lt_plot];
dist_row = norm(p1 - p2);
mouth_row=mean_row+ceil(1.8*dist_row);
mouth_col=ceil((col_rt_plot+col_lt_plot)/2);
plot(mouth_col,mouth_row,'c*');
hold off;
%       
