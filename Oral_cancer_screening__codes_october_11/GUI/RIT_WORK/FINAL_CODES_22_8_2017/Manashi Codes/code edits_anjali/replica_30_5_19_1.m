...
rightROI = imread([pathname,'Auto_ROI_Front_',nameFolds,'\A_ROI_R.jpg']); %(MASKED)
%rightROI = imread('Auto_ROI','A_ROI_R.jpg');
IR= imcrop(rightROI,[50 130 348 320]);
axes(handles.axes2)
imshow(IR);
leftROI = imread([pathname,'Auto_ROI_Front_',nameFolds,'\A_ROI_L.jpg']);
IL= imcrop(leftROI,[200 140 348 320]);
axes(handles.axes3)
imshow(IL);
% V= filename(1:end-4);
