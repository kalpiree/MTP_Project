...
% Last Modified by GUIDE v2.5 02-May-2018 11:58:24
...
[A_ROI_R,A_ROI_L]=Face_Detection_Keypoint_Cleaned_Code_new(filename,pathname);
%(MASKED)
%[A_ROI_R,A_ROI_L]=Face_Detection_Keypoint_Cleaned_Code %(MINE)
%addpath 'D:\anjali\17EC65R09_Abhishek\Codes\Code Repository\Face
%Detection\'; (MINE)

%Face_Detection_Keypoint_Cleaned_Code_new %(MASKED)
%rightROI = imread([pathname,'Auto_ROI','\A_ROI_R.jpg']); (MASKED)
rightROI = imread('Auto_ROI','A_ROI_R.jpg');
I= imcrop(rightROI,[50 130 348 320]);
axes(handles.axes2)
imshow(I);
leftROI = imread([pathname,'Auto_ROI','\A_ROI_L.jpg']);
I= imcrop(leftROI,[200 140 348 320]);
axes(handles.axes3)
imshow(I);
% V= filename(1:end-4);
...
[GT_gabor_F_normal_lt, GT_gabor_F_normal_rt]= feature_extraction_classification_GUI(I1,mask_lt,mask_rt )
