...
reqd_index_rt=0;
neg_count=0;
overall_positive_counter=0;
positive_counter=0;
    if(corner_angle_rt(neck_index_rt)>=0)
        while(((corner_angle_rt(neck_index_rt)>=0) || neg_count<2)&& neck_index_rt~=size(corner_angle_rt,2)  )
           if(corner_angle_rt(neck_index_rt)<0)
                neg_count=neg_count+1;
                if(corner_angle_rt(neck_index_rt+1)<0)
                    neg_count=2;
                    positive_counter=0;
                    while(corner_angle_rt(neck_index_rt)<0 &&  neck_index_rt~=size(corner_angle_rt,2))
            ...
            if(positive_counter>overall_positive_counter)
                overall_positive_counter=positive_counter;
                reqd_index_rt=neck_index_rt;
            end
           end
            neck_index_rt=neck_index_rt+1;
        ...
        while(((corner_angle_rt(neck_index_rt)>=0) || neg_count<2)&& neck_index_rt~=size(corner_angle_rt,2)  )
            if(corner_angle_rt(neck_index_rt)<0)
                ...
                positive_counter=positive_counter+1;
            if(positive_counter>overall_positive_counter && neck_index_rt~=size(corner_angle_rt,2))
                overall_positive_counter=positive_counter;
                reqd_index_rt=neck_index_rt;
            end
            end
    ...
    neck_index_rt=reqd_index_rt;
         %% Left Face Corners
neck_index_lt=1;
pos_count=0;
overall_negative_counter=0;
negative_counter=0;
    if(corner_angle_lt(neck_index_lt)<=0)
        while(((corner_angle_lt(neck_index_lt)<=0) || pos_count<2)&& neck_index_lt~=size(corner_angle_lt,2)  )
           if(corner_angle_lt(neck_index_lt)>0)
                pos_count=pos_count+1;
                if(corner_angle_lt(neck_index_lt+1)>0)
                    pos_count=2;
                    negative_counter=0;
                    while(corner_angle_lt(neck_index_lt)<0 &&  neck_index_lt~=size(corner_angle_lt,2))
            ...
            if(negative_counter>=overall_negative_counter)
                overall_negative_counter=negative_counter;
                reqd_index_lt=neck_index_lt;
            end
           end
            neck_index_lt=neck_index_lt+1;
        ...
        while(((corner_angle_lt(neck_index_lt)>=0) || pos_count<2)&& neck_index_lt~=size(corner_angle_lt,2)  )
            if(corner_angle_lt(neck_index_lt)>0)
                ...
                negative_counter=negative_counter+1;
            if(negative_counter>=overall_negative_counter && neck_index_lt~=size(corner_angle_lt,2))
                overall_negative_counter=negative_counter;
                reqd_index_lt=neck_index_lt;
            end
            end
