...
% Last Modified by GUIDE v2.5 19-Sep-2019 15:05:49
...
cd '..\';
current_d=dir;
current_d=current_d(~ismember({current_d.name},{'.','..'}));
%f=current_d.name;
%l=length(current_d);

for i=1:length(current_d)
    %foldername=current_d(i).name;
    if i~=length(current_d)
        if current_d(i).name==p_id
            new_d=current_d(i+1).name;
            %disp(new_d.name);
        end
   
    else                                    % ie, if i==length(current_d)
        if current_d(i).name==p_id
            new_d=current_d(1).name;
            %disp(new_d);
        end
    end       
end 
...
%d=d(~ismember({d.name},{'.','..'}));
%disp(d);
set(handles.edit1,'String',new_d);
csvNameRT = new_d;
right= xlsread([csvNameRT,'_rt.csv']); % reads the .csv file corresponding to right profile view
maxval = max(max(right));   
minval = min(min(right));
range = 1/(maxval - minval);
right = range .*(right - minval); % normalization of image to make the pixel values to a range between 0 and 1 (from their original range between 0 to 255)
...
csvNameFT=new_d;
data=xlsread([csvNameFT,'.csv']); % reads the .csv file corresponding to front view
maxval = max(max(data));
minval = min(min(data));
range = 1/(maxval - minval);
data = range .*(data - minval);
...
csvNameLT =new_d;
left= xlsread([csvNameLT,'_lt.csv']); % reads the .csv file corresponding to left profile view
maxval = max(max(left));
minval = min(min(left));
range = 1/(maxval - minval);
left = range .*(left - minval);
