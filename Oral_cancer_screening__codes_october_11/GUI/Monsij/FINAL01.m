...
cd(pathname);
%disp(pathname);
cd 'C:\Users\ntnbs\Downloads\Oral_cancer_screening_previous_codes\Code Repository\Face Detection\';
Face_Detection_Keypoint_Cleaned_Code_new

cd(pathname);
cd 'C:\Users\ntnbs\Downloads\Oral_cancer_screening_previous_codes\Code Repository\Profile Face Detection\';
...
answer=questdlg('ROI extracted and saved. Do you wish to make changes in the ROI?', 'Message', 'Yes', 'No, thanks','No, thanks' );
    switch answer
        case 'Yes'
            ...
            set(handles.pushbutton14,'Enable','on');
            set(handles.pushbutton15,'Enable','on');
            f=msgbox('Click on the respective Redraw/Modify ROI buttons');
            
        case 'No, thanks'
            %cd(pathname);
            %disp(pathname);
            cd('C:\Users\ntnbs\Downloads\Oral_cancer_screening_previous_codes\Code Repository\GUI\Monsij');
            set(handles.pushbutton1,'Enable','on');
...
%handles.filename = filename;
   pathname=handles.pathname;
    ...
    %disp(out);
    %name1=out(end-2);
    %set(handles.edit3,'String',name1);
 ...
 cd('C:\Users\ntnbs\Downloads\Oral_cancer_screening_previous_codes\Code Repository\Multiresolution\Classification\libsvm-3.20\matlab\GaborResults\AutomatedROI\AROI_Full_Gabor_MN_Frontal')
