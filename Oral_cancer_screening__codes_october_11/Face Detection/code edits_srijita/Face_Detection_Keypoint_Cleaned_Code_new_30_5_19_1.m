...
for folder_idx= no_dir:no_dir
    ...
    fprintf('Processing Patient ID '); disp(filename); % Display Current Patient ID
    % Load the required Thermal Image
    %% I_F is the loaded image (from excel file)(MASKED)
    % filename is the loaded image (form excel file) (MINE)
    %if (strcmp(User_Choice, 'N')==1)
    %%   I_F=xlsread(['..\..\ThermalDatabase\Normal\',nameFolds{folder_idx},'\',nameFolds{folder_idx},'.csv']);(MASKED)
      %  filename=xlsread(['..\..\ThermalDatabase\Normal\',nameFolds{logical_folder},'\',nameFolds{logical_folder},'.csv']);(MINE)
      %%  rel_path=['..\..\ThermalDatabase\Normal\',nameFolds{folder_idx}];(MASKED)
       % rel_path=['..\..\ThermalDatabase\Normal\',nameFolds{logical_folder}];(MINE)
       %% folder_name=['A_ROI_Front_' nameFolds{folder_idx}];(MASKED)
        %folder_name=['A_ROI_Front_' nameFolds{logical_folder}];(MINE)
        
    %elseif (strcmp(User_Choice, 'A')==1)
     %%   I_F=xlsread(['..\..\ThermalDatabase\Normal_AgeAdjust\',nameFolds{folder_idx},'\',nameFolds{folder_idx},'.csv']);
     %   (MASKED)
     %   filename=xlsread(['..\..\ThermalDatabase\Normal_AgeAdjust\',nameFolds{logical_folder},'\',nameFolds{logical_folder},'.csv']);(MINE)
      %%  rel_path=['..\..\ThermalDatabase\Normal_AgeAdjust\',nameFolds{folder_idx}];(MASKED)
       %  rel_path=['..\..\ThermalDatabase\Normal_AgeAdjust\',nameFolds{logical_folder}];(MINE)
       %% folder_name=['A_ROI_Front_' nameFolds{folder_idx}];(MASKED)
        % folder_name=['A_ROI_Front_' nameFolds{logical_folder}];(MINE)
    %elseif (strcmp(User_Choice, 'M')==1)
     %%   I_F=xlsread(['..\..\ThermalDatabase\Malignant\',nameFolds{folder_idx},'\',nameFolds{folder_idx},'.csv']);(MASKED)
      %  filename=xlsread(['..\..\ThermalDatabase\Malignant\',nameFolds{logical_folder},'\',nameFolds{logical_folder},'.csv']);(MINE)
      %%  rel_path=['..\..\ThermalDatabase\Malignant\',nameFolds{folder_idx}];(MASKED)
      %  rel_path=['..\..\ThermalDatabase\Malignant\',nameFolds{logical_folder}];(MINE)
       %% folder_name=['A_ROI_Front_' nameFolds{folder_idx}];(MASKED)
       % folder_name=['A_ROI_Front_' nameFolds{logical_folder}];(MINE)
    %elseif (strcmp(User_Choice, 'P')==1)
     %%   I_F=xlsread(['..\..\ThermalDatabase\NonMalignant\',nameFolds{folder_idx},'\',nameFolds{folder_idx},'.csv']);
     %   (MASKED)
     %   filename=xlsread(['..\..\ThermalDatabase\NonMalignant\',nameFolds{logical_folder},'\',nameFolds{logical_folder},'.csv']);(MINE)
      %%  rel_path=['..\..\ThermalDatabase\NonMalignant\',nameFolds{folder_idx}];(MASKED)
      %  rel_path=['..\..\ThermalDatabase\NonMalignant\',nameFolds{logical_folder}];(MINE)
       %% folder_name=['A_ROI_Front_' nameFolds{folder_idx}];(MASKED)
        % folder_name=['A_ROI_Front_' nameFolds{logical_folder}];(MINE)
    %else
     %   disp('Invalid Choice');
    %end
    %% Face Detection
    %%  I/P = Input image (I_F)(MASKED)
    %  filename = Input image (filename)(MINE)
    %  O/P = Face Boundary Mask (BW_mask_F)
    %  Function Used = face_detect_new.m
    
    disp('Face Detection - Started');
    %I_F_cpy=filename;
    logical_folder=logical(folder_idx);
    op_img1=nameFolds(logical_folder); % Storing Patient ID
    %[BW_mask_F,mask_F,rt_boundary_col,lt_boundary_col,~,lower_boundary_row]=face_detect_new( filename,nameFolds,folder_idx,op_img1 );% Perform Face-Detection
    [BW_mask_F,mask_F,rt_boundary_col,lt_boundary_col,~,lower_boundary_row]=face_detect_new( filename,folder_idx,op_img1);
    lower_boundary_row=floor(lower_boundary_row);
    filename=mat2gray(filename);
    face_img=(filename).*double(BW_mask_F); % Convolving the face detected mask with the original image to obtain the face.
