...
figure();
imshow(face_img);hold on;plot(col_rt_eye,row_rt_eye,'r*',col_lt_eye,row_lt_eye,'r*');
%% Mouth Detection
...
Mouth_Region_Image=face_img(req_row:lower_lip_row,req_col_mouth_rt:req_col_mouth_lt);
 figure();imshow(face_img);hold on;
    rectangle('Position', [req_col_mouth_rt,req_row,req_col_mouth_lt-req_col_mouth_rt,lower_lip_row-req_row],...
...
fullFileName = fullfile('C:\Users\chaithan\Desktop\Normal1', op_img1);
...
rel_path=['..\..\Normal\',nameFolds{folder_idx}];
            cd(rel_path);
