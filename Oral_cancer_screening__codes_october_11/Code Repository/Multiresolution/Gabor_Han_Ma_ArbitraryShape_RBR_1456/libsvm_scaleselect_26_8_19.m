...
function [Final_accuracy,indices,optns,save_optns,save_prob_estimates,model,save_idx_validate,save_True_Label_test,save_Predict_label_test,save_best_kernel_model,save_bst_kernel_idx,save_data_test,save_data_train,save_data_validate,avg_sen,avg_spc,counter]= libsvm_scaleselect_26_8_19(xdata_allscale,Target_train,no_of_folds,scales,viz_case,Flag,Setting,nameFolds)
group = Target_train;
...
if(Flag =='1')
    ...
    else(strcmp(Setting,'AROI_PN_AGE')==1)
...
%     specificity(fold_no)=TN/(TN+FP);
 ...
 Final_accuracy=sum(final_accuracy)/5;
 avg_sen=sum(Sensitivity)/5;
 avg_spc=sum(Specificity)/5;
 avg_distance = sum(distance)/5;
