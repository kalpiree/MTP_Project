%% Face Localization, Keypoint Detection and Automatic ROI formation for Thermal Images - Oral Cancer
    ...
    function [A_ROI_R,A_ROI_L]=Face_Detection_Keypoint_Cleaned_Code(filename,pathname)
dbstop if error
%close all
...
%% Face Detection
%  I/P = Input image (I_F) is in csv
%  O/P = Face Boundary Mask (BW_mask_F)is in binary array
...
%      disp(pathname);
    ...
    I_F=xlsread( [pathname,'\',filename]);
     I_F_cpy=I_F;
      % Storing Patient ID
     [BW_mask_F,mask_F,rt_boundary_col,lt_boundary_col,~,lower_boundary_row]=face_detect( I_F); % Perform Face-Detection
...
[ face_img ] = Connected_Components_Face( face_img,lower_lip_row );
%disp('Cloth Pixels Removed');

%% ====== Start of Hull Mask =====%%%
% This part of the code removes themouth reigonofthe faceas this reigon
% gives false hotspot
...
left= 'A_ROI_L.jpg';
right= 'A_ROI_R.jpg';
fulldestination1 = fullfile(s, left);
fulldestination2 = fullfile(s, right);
imwrite(A_ROI_L,fulldestination1);
imwrite(A_ROI_R,fulldestination2);
cd 'C:\Users\Admin-16\Documents\MATLAB';


% save(A_ROI_R.jpg,'A_ROI_R');
% save(A_ROI_L.jpg,'A_ROI_L');
%%%%here you can save the roi
%



% direc=['E:\FINAL_CODES_22_8_2017\Manashi Codes\ThermalDatabase\NonMalignant\',nameFolds{folder_idx},'\A_ROI_Front_',nameFolds{folder_idx},'\A_ROI_Front.mat'];
% Folder_Path=['E:\FINAL_CODES_22_8_2017\Manashi Codes\ThermalDatabase\NonMalignant\',nameFolds{folder_idx},'\A_ROI_Front_',nameFolds{folder_idx}];
% mkdir(Folder_Path);
% save(direc,'A_ROI_R','A_ROI_L');




% %% Display
% % Display all the keypoints on the detected face image.
%  close all;
% [r,c]=find(face_img);
% rmax=max(r);
% figure;
...
% plot(mid_mouth,lower_lip_row,'y*');
% figure();
% imshow(face_img); hold on;
% vec=find(face_img(Lower_eye_row,:)>0);
% start=min(vec);
% stop=max(vec);
% plot([start:stop],Lower_eye_row,'b*');
% k = convhull(face_keypt_X,face_keypt_Y);
% plot(face_keypt_X(k),face_keypt_Y(k),'r-');
% figure();
% imshow(A_ROI_R,[]);title(nameFolds{folder_idx});
% figure();
% imshow(A_ROI_L,[]);title(nameFolds{folder_idx});
 end  